...
export type ForecastData = {
  ...
  // Optional multi-concept override. When present and non-empty, the report
  // renders one ad card per entry instead of the single hero_concept, each
  // pulling its own image via that concept's image_path filename. Added for
  // Shop Dreamers of Dreams (2026-07-20), where three angles were pitched
  // side by side. Reports without this field are unaffected.
  hero_concepts?: HeroConcept[]
  next_step: {
    urgency: string
    headline: string
    body: string
...
function mockupImageExists(slug: string, file = "hero-mockup.jpg"): boolean {
  const p = path.join(process.cwd(), "public", "creatives", slug, file)
  return fs.existsSync(p)
}

// A concept's image_path may carry a bare filename (multi-concept reports write
// hero-mockup-2.jpg, hero-mockup-3.jpg). Anything else falls back to the
// single-concept default so existing reports keep resolving.
function mockupFileFor(hero: HeroConcept): string {
  const p = hero.image_path
  if (p && /^hero-mockup(-\d+)?\.jpg$/.test(p)) return p
  return "hero-mockup.jpg"
}
...
export default function ForecastTemplate({
  ...
  const heroes: HeroConcept[] =
    data.hero_concepts && data.hero_concepts.length > 0
      ? data.hero_concepts
      : data.hero_concept
        ? [data.hero_concept]
        : []
    ...
    <div className="wrap">
      ...
      {heroes.length > 0 && (
        <>
          <h2>
            {heroes.length > 1
              ? `Here are ${heroes.length} ads we'd run for you (concepts)`
              : "Here's the ad we'd run for you (concept)"}
          </h2>
          <div className="hero-mockup-intro">
            {heroes.length > 1
              ? "Three different angles on the same problem, built from your own product photos - once we work from your raw assets, our team ships 10-20 production-ready variations (including videos) like this every week."
              : "Built from your own product photos to show direction - once we work from your raw assets, our team ships 10-20 production-ready variations (including videos) like this every week."}
          </div>
          {heroes.map((hero, hi) => {
          const heroImageReady = hero.image_path && mockupImageExists(slug, mockupFileFor(hero))
          return (
          <div className="hero-mockup" key={hi}>
            <div className="fb-ad">
              ...
              <div className="fb-ad-image">
                {heroImageReady ? (
                  <img src={`/creatives/${slug}/${mockupFileFor(hero)}`} alt={hero.concept_name} />
                ) : (
